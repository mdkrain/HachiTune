...
std::pair<int, int>
IncrementalSynthesizer::expandToSilenceBoundaries(int dirtyStart,
                                                  int dirtyEnd) {
  if (!project)
  ...
  if (expandedStart > dirtyStart)
    expandedStart = dirtyStart;
  ...
  if (expandedEnd < dirtyEnd)
    expandedEnd = dirtyEnd;
