...
float PianoRollRenderer::catmullRom(float t, float p0, float p1, float p2,
                                    float p3) {
  float t2 = t * t;
  float t3 = t2 * t;
...
void PianoRollRenderer::drawBackgroundWaveform(
  ...
  bool cacheValid =
      waveformCache.isValid() && std::abs(cachedScrollX - scrollX) < 1.0 &&
      std::abs(cachedPixelsPerSecond - pixelsPerSecond) < 0.01f &&
      cachedWidth == area.getWidth() && cachedHeight == area.getHeight();
  ...
  // Render to cache
  waveformCache =
      juce::Image(juce::Image::ARGB, area.getWidth(), area.getHeight(), true);
...
void PianoRollRenderer::drawTimeline(juce::Graphics &g, int width) {
  ...
  for (float time = 0.0f; time <= duration + secondsPerTick;
       time += secondsPerTick) {
    float x = CoordinateMapper::pianoKeysWidth + time * pixelsPerSecond -
...
void PianoRollRenderer::drawNotes(juce::Graphics &g, double visibleStartTime,
  ...
  for (auto &note : project->getNotes()) {
    ...
    float x =
        static_cast<float>(noteStartTime * coordMapper->getPixelsPerSecond());
    float w = framesToSeconds(note.getDurationFrames()) *
    ...
    float pitchOffsetPixels =
        -note.getPitchOffset() * coordMapper->getPixelsPerSemitone();
...
void PianoRollRenderer::drawNoteWaveform(juce::Graphics &g, const Note &note,
  ...
  for (size_t i = 0; i + 1 < numPoints; ++i) {
    ...
    for (int seg = 1; seg <= curveSegments; ++seg) {
      ...
      float val = catmullRom(t, waveValues[idx0], waveValues[idx1],
                             waveValues[idx2], waveValues[idx3]);
  ...
  // Bottom curve
  waveformPath.lineTo(x + w,
                      centerY + waveValues[numPoints - 1] * waveHeight * 0.5f);
...
void PianoRollRenderer::drawPitchCurves(juce::Graphics &g,
  ...
  for (const auto &note : project->getNotes()) {
    ...
    for (int i = startFrame; i < endFrame; ++i) {
      ...
      if (finalMidi > 0.0f) {
        ...
        float y = coordMapper->midiToY(finalMidi) +
                  coordMapper->getPixelsPerSemitone() * 0.5f;
...
void PianoRollRenderer::drawCursor(juce::Graphics &g, double cursorTime,
                                   int height) {
  if (!coordMapper)
    return;
